...
/** 1. **/
/** a. **/
SELECT m.idMarinheiro, nome, COUNT(r.idMarinheiro) "Qtd_Reservas" 
FROM marinheiro m
FULL OUTER JOIN reserva r ON r.idMarinheiro=m.idMarinheiro
GROUP BY m.idMarinheiro,m.nome
ORDER BY "Qtd_Reservas" DESC;

/** b. **/
SELECT idMarinheiro, nome, (SELECT COUNT(idMarinheiro) 
FROM reserva
WHERE idMarinheiro=marinheiro.idMarinheiro) qtd_reserva 
FROM marinheiro
ORDER BY qtd_reserva DESC;

/** 2. **/
SELECT DISTINCT idMarinheiro,idBarco,COUNT(*) 
FROM reserva r1
GROUP BY idMarinheiro,idBarco
HAVING COUNT(idbarco)>(
SELECT AVG(COUNT(idBarco))
FROM reserva r2
WHERE r2.idBarco=r1.idBarco
GROUP BY idBarco,idmarinheiro);
...
WHERE idMarinheiro=marinheiro.idMarinheiro) qtd_reserva 
FROM marinheiro
WHERE (SELECT COUNT(idMarinheiro) 
FROM reserva
WHERE idMarinheiro=marinheiro.idMarinheiro) IN (SELECT DISTINCT COUNT(idMarinheiro) 
FROM reserva
GROUP BY idMarinheiro
ORDER BY COUNT(idMarinheiro) DESC 
