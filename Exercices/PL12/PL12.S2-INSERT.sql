...
FROM medico m
JOIN consulta c ON c.idMedico=m.idMedico
WHERE 3 < ALL (
    SELECT COUNT(mr.idMedicamento) "medicamentos por consulta"
    FROM consulta c1
    LEFT JOIN medicamentoReceitado mr ON mr.idConsulta=c1.idConsulta
    WHERE c1.idMedico=m.idMedico
    GROUP BY c1.idConsulta
)
ORDER BY m.nome;

/** 2. **/
SELECT DISTINCT m.nome, (SELECT COUNT(DISTINCT mr.idMedicamento) "medicamentos por consulta"
                        FROM medicamentoReceitado mr 
                        INNER JOIN consulta c ON c.idConsulta=mr.idConsulta) "NR_MEDICAMENTOS_RECEITADOS"
FROM medico m
INNER JOIN consulta c ON c.idMedico=m.idMedico
GROUP BY m.nome
HAVING m.idMedico = ALL ( SELECT me.idMedico
                            FROM medico me
                            JOIN consulta cs ON cs.idMedico=me.idMedico
                            AND cs.idConsulta = ALL (SELECT con.idConsulta
                                                        FROM consulta con
                                                        JOIN medicamentoReceitado mr ON con.idConsulta=mr.idConsulta))