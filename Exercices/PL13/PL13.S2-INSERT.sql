...
-- test
BEGIN
    DBMS_OUTPUT.PUT_LINE('Total de clientes: ' || fncTotalClientes());
END;

/** 6. **/
CREATE OR REPLACE FUNCTION fncTemLivrosEditora
(v_id_editora Editora.idEditora%type) RETURN BOOLEAN
IS
    v_checker INTEGER;
    v_stock INT;
    parametro_invalido EXCEPTION;
BEGIN
    SELECT COUNT(*) INTO v_checker
    FROM Editora
    WHERE idEditora=v_id_editora;
    IF v_checker = 0 THEN
    ...
    FROM EdicaoLivro el
    WHERE el.idEditora = v_id_editora;
    RETURN v_stock != 0;
...
-- tests
BEGIN
    DBMS_OUTPUT.PUT_LINE('Editora 700 tem livros? '|| sys.diutil.bool_to_int(fncTemLivrosEditora(700)));
end;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Editora 1400 tem livros? '|| sys.diutil.bool_to_int(fncTemLivrosEditora(1400)));
end;
       
/** 7.**/
CREATE OR REPLACE FUNCTION  fncClienteInfo
(v_nif_cliente Cliente.nifCliente%type) RETURN STRING
IS
    v_info_cliente STRING(300);
    v_cod_postal CodigoPostal.codPostal%type;
    v_localidade CodigoPostal.localidade%type;
    v_nome Cliente.nome%type;
    v_data_nascimento Cliente.dataNascimento%type;
    v_morada Cliente.morada%type;
    v_nr_telemovel Cliente.nrTelemovel%type;
    v_checker INTEGER;
    parametro_invalido EXCEPTION;
BEGIN
    SELECT COUNT(*) INTO v_checker
    FROM Cliente
    WHERE nifCliente=v_nif_cliente;
    IF v_checker = 0 THEN
        RAISE parametro_invalido;
    END IF;
    ...
    FROM Cliente
    WHERE nifCliente=v_nif_cliente;
    v_info_cliente := v_info_cliente || 'Nome: ' || v_nome || CHR(10);
    
    -- NIF
    v_info_cliente :=  v_info_cliente || 'NIF: ' || v_nif_cliente || CHR(10);
    
    SELECT dataNascimento INTO v_data_nascimento
    FROM Cliente
    WHERE nifCliente=v_nif_cliente;
    v_info_cliente := v_info_cliente || 'Data de nascimento: ' || v_data_nascimento || CHR(10);
    
    -- Morada
    SELECT morada INTO v_morada
    FROM Cliente
    WHERE nifCliente=v_nif_cliente;
    v_info_cliente := v_info_cliente || 'Morada: ' || v_morada || CHR(10);
    
    -- Código Postal e Localidade
    SELECT codPostal INTO v_cod_postal
    FROM Cliente
    WHERE nifCliente=v_nif_cliente;
    SELECT cp.localidade INTO v_localidade
    FROM Cliente c, CodigoPostal cp
    WHERE c.nifCliente=v_nif_cliente AND c.codPostal=cp.codPostal;
    v_info_cliente := v_info_cliente || 'Código Postal: ' || v_cod_postal || ' ' || v_localidade || CHR(10);
    
    -- Número de telemóvel
    SELECT nrTelemovel INTO v_nr_telemovel
    FROM Cliente
    WHERE nifCliente=v_nif_cliente;
    v_info_cliente := v_info_cliente || 'Telemóvel: ' || v_nr_telemovel;

    RETURN v_info_cliente;
...
-- tests
BEGIN
    DBMS_OUTPUT.PUT_LINE('Informações do cliente 123456789:'||CHR(10)||fncClienteInfo(123456789));
end;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Informações do cliente 900800100:'||CHR(10)||fncClienteInfo(900800100));
end;

/** 8. **/
CREATE OR REPLACE FUNCTION fncStockAnoEditora
(v_id_editora EdicaoLivro.idEditora%type, v_ano EdicaoLivro.anoEdicao%type)
RETURN EdicaoLivro.stock%type
IS
...
BEGIN
    ...
    IF v_check = 0 OR v_ano > EXTRACT(YEAR FROM sysdate) OR v_ano < 1450 THEN
        RAISE parametro_invalido;
    END IF;

    SELECT SUM(stock) INTO v_stock
    FROM EdicaoLivro
    WHERE anoEdicao=v_ano
    AND idEditora=v_id_editora;
    RETURN v_stock;
...
-- tests
BEGIN
    DBMS_OUTPUT.PUT_LINE('Stock para Editora 8 em 1999: ' || fncStockAnoEditora(8,1999));
END;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Stock para Editora 1500 em 2022: ' || fncStockAnoEditora(1500,2022));
END;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Stock para Editora 1500 em 1999: ' || fncStockAnoEditora(1500,1999));
END;

/** 9. **/
CREATE OR REPLACE FUNCTION fncPrecoVenda
(v_nr_venda Venda.nrVenda%type)
RETURN PrecoEdicaoLivro.preco%type
IS
    v_isbn Venda.isbn%type;
    v_quantidade_venda Venda.quantidade%type;
    v_preco_edicao_livro PrecoEdicaoLivro.preco%type;
    v_check INTEGER;
    parametro_invalido EXCEPTION;
BEGIN
    SELECT COUNT(*) INTO v_check
    FROM Venda
    WHERE nrVenda=v_nr_venda;
    IF v_check = 0 THEN
        RAISE parametro_invalido;
    END IF;
    ...
    SELECT quantidade INTO v_quantidade_venda
    FROM Venda
    WHERE nrVenda=v_nr_venda;
    
    SELECT preco INTO v_preco_edicao_livro
    FROM PrecoEdicaoLivro
    WHERE isbn=v_isbn
    AND dataInicio = (
        SELECT MAX(dataInicio)
        FROM PrecoEdicaoLivro pel
        WHERE pel.isbn=v_isbn
    );
    
    RETURN v_preco_edicao_livro * v_quantidade_venda;
...
-- tests
SELECT v.nrVenda, v.isbn, pel.preco "preco edicao", v.quantidade "quant", pel.preco*v.quantidade "preco a pagar"
FROM Venda v
JOIN PrecoEdicaoLivro pel ON pel.isbn=v.isbn
WHERE pel.dataInicio=(
    SELECT MAX(dataInicio)
    FROM PrecoEdicaoLivro pel1
    WHERE pel1.isbn=v.isbn
)
ORDER BY v.nrVenda;

BEGIN
    DBMS_OUTPUT.PUT_LINE('Preço da venda 306: ' || fncPrecoVenda(306)); 
END;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Preço da venda 1: ' || fncPrecoVenda(1)); 
END;